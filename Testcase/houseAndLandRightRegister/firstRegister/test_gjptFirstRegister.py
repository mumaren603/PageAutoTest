...
class Test_gjptFirstRegister():
    ...
    def test_gjptFristRegister(self,login,cmdopt):
        ...
        dbInfo = login[1]
        # 获取办件数据
        bdcdyh = dataInit(dbInfo).getGjptFirstRegisterData()
        ...
        if qlrmc:
            try:
                resDataCheck = dataResCheck(dbInfo).gjptRegisterDataCheck(bdcdyh)
...
if __name__ == '__main__':
    pytest.main(['-v', 'test_gjptFristRegister'])
